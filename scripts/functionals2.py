...
def simple_metric_comparison(radiomics_bl,baseline,deltaV_perc,inds_noOutliers,featuresOfInterest,tr=50):
    ...
    volMetrics['Patient Age'] = baseline.AGE[inds_noOutliers]
...
# def mannu_testing(result,pThresh=0.25):
#     arm = [0,1]
#     metrics = ['Intensity','Entropy']
#     morphs = ['whole lesion','lesion core','interior rim','exterior rim']

#     for i in range(len(arm)):                  # for each arm
#         arm_inds = result.Arm == arm[i]
    
#         for j in range(len(metrics)):          # for each metric
#             metric_inds = result.Metric == metrics[j]
#             combo_inds = np.logical_and(arm_inds,metric_inds)
        
#             for k in range(len(morphs)):       # for each morphological region
#                 morph_inds = result.Region == morphs[k]
#                 all_inds = np.logical_and(combo_inds,morph_inds)
            
#                 subset = result[all_inds]
#                 responders = subset[subset.Response == -1].Value.values
#                 stable = subset[subset.Response == 0].Value.values
#                 progressors = subset[subset.Response == +1].Value.values
#                 Urp,prp = mannwhitneyu(responders.astype('float'),progressors.astype('float'))
#                 Urs,prs = mannwhitneyu(responders.astype('float'),stable.astype('float'))
#                 Usp,psp = mannwhitneyu(stable.astype('float'),progressors.astype('float'))

#                 if prp < pThresh:
#                     print('Arm: {}; '.format(arm[i]),'Metric: {}; '.format(metrics[j]),'Region: {}'.format(morphs[k]))
#                     print('Responder/Progressor p-value: {:2f}'.format(prp))
#                 if prs < pThresh:
#                     print('Arm: {}; '.format(arm[i]),'Metric: {}; '.format(metrics[j]),'Region: {}'.format(morphs[k]))
#                     print('Responder/Stable p-value: {:2f}'.format(prs))
#                 if psp < pThresh:
#                     print('Arm: {}; '.format(arm[i]),'Metric: {}; '.format(metrics[j]),'Region: {}'.format(morphs[k]))
#                     print('Stable/Progressor p-value: {:2f}'.format(psp))
...
def draw_cv_roc_curve(classifier, cv, X, y, title='ROC Curve'):
    ...
    plt.plot(mean_fpr, mean_tpr, color='#CC79A7',
    ...
    plt.xlim([-0.05, 1.05])
    plt.ylim([-0.05, 1.05])
    plt.xlabel('False Positive Rate', color = '#CC79A7')
    plt.ylabel('True Positive Rate', color = '#CC79A7')
    # plt.title(title)
    plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
    
    for ax in plt.gcf().axes:
        ax.get_lines()[0].set_color("#CC79A7")
    plt.savefig('roc.png',transparent=True,dpi=500)
    plt.show()
...
def draw_cv_pr_curve(classifier, cv, X, y, title='PR Curve'):
    ...
    for train, test in cv.split(X, y):
        ...
        aps.append(average_precision_score(y[test], probas_[:,1]))
        mcc.append(matthews_corrcoef(y[test], probas_[:,1]>0.5))
    ...
    plt.plot(recall, precision, color='b',
             ...
             lw=2, alpha=.8)
    perc = np.sum(y_real)/len(y_real)
    plt.plot([0,1], [perc,perc], linestyle='--', lw=2, color='r',
    ...
    return average_precision_score(y_real, y_proba), matthews_corrcoef(y_real, y_proba>0.5),aps,mcc,y_real,y_proba
